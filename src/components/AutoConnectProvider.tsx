...
export const AutoConnectProvider: FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isAutoConnect) {
        const autoConnectValue = JSON.parse(isAutoConnect);
        console.log("Auto-connect loaded from localStorage:", autoConnectValue);
        return setAutoConnect(autoConnectValue);
      }
    } catch (e) {
